// LUT parsing and sampling.
//
// Images are graded in a worker and video on the main thread (a <video> element
// cannot be decoded off-document), so this lives on its own and both import it.
// One implementation means a look cannot drift between a photograph and a frame.
...
export function sampleLut(lut, r, g, b) {
  ...
  for (let corner = 0; corner < 8; corner++) {
    ...
    const weight = (useR1 ? dr : 1 - dr) * (useG1 ? dg : 1 - dg) * (useB1 ? db : 1 - db);
...
/**
 * Apply a LUT to RGBA pixel data in place.
 *
 * @param {Uint8ClampedArray} data
 * @param {number} intensity  0 leaves the pixels alone, 1 applies fully.
 */
export function applyLutToPixels(data, lut, intensity = 1) {
